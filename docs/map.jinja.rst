...
We will see a quick setup to configure the ``dnsmasq`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``dnsmasq`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``dnsmasq`` formulas under ``/srv/salt/dnsmasq/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/dnsmasq/parameters/dns:domain/
    ln -s dns:domain /srv/salt/dnsmasq/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/dnsmasq/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/dnsmasq/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``dnsmasq`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``dnsmasq`` formulas under ``/srv/salt/dnsmasq/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/dnsmasq/parameters/roles
...
We create a configuration for the role ``dnsmasq/server`` in ``/srv/salt/dnsmasq/parameters/roles/dnsmasq/server.yaml``:
...
We create another configuration for the role ``dnsmasq/client`` in ``/srv/salt/dnsmasq/parameters/roles/dnsmasq/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles dnsmasq/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles dnsmasq/client
...
.. code-block:: console
    ...
    mv /srv/salt/dnsmasq/parameters/dns:domain/  '/srv/salt/dnsmasq/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-dnsmasq-as-json:
      test.nop:
        - name: {{ dnsmasq | json }}
...
Here is an example based on `template-formula/dnsmasq/config/file.sls`_:
...
.. code-block:: sls
    ...
    dnsmasq-config-file-file-managed:
      file.managed:
        - name: {{ dnsmasq.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='dnsmasq-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ dnsmasq.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            dnsmasq: {{ dnsmasq | json }}

This ``sls`` file expose a ``dnsmasq`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ dnsmasq['winner'] }}
...
.. _template-formula/dnsmasq/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/dnsmasq/config/file.sls
